...
def dashboard2():
    ...
    # Convert fetched data into product list for the template
    products = []
    for item in fetch_product:
        ...
        products.append({
            "brand": brand,  # You can update this based on data if needed
            "name": name,
            "price": price,
            "image": "1.jpg",
            "product_id": str(product_id)
...
def place_bid():
    ...
    place_bid = api.place_bid_price(product_id, bid_amount, user_id="999")

    print("place_bid: ", place_bid)

    # Your logic to handle bid
    print(f"Received bid of ₹{bid_amount} for product {product_id}")

    return jsonify({"message": "Bid successfully placed!"})
...
def buynow():
    ...
    images = raw_cart_products[5].split(",")  # Split into list
    image = images[0].strip()  # Use only the first image
...
def favourite():
    ...
    return render_template("favourite.html", products=products)
...
def cart():
    ...
    for item in raw_cart_products:
        ...
        total_price += price * qty  # Add to total
        ...
        cart_products.append({
            ...
            'price': price,
            'qty': qty,
            'image': '1.jpg'  # Replace if actual image exists
        })
...
def remove_from_cart(product_id):
    ...
    user_id = '999'  # Replace with session or actual user ID
    result = api.remove_product_from_cart(product_id, user_id)  # Call your actual DB/API function
    print("result ", result)
...
def search_products():
    products = [
        {"id": 1, "image": "product.jpg", "brand": "Nike", "name": "Air Max", "price": "₹120"},
        {"id": 2, "image": "product.jpg", "brand": "Adidas", "name": "Ultraboost", "price": "₹140"},
    ]
